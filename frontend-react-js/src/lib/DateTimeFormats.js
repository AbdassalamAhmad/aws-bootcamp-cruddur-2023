...
export function time_ago(value){
  ...
  const now     = DateTime.now()
  const diff_mins = Math.abs(future.diff(now, 'minutes').toObject().minutes);
  const diff_hours = Math.abs(future.diff(now, 'hours').toObject().hours);
  const diff_days = Math.abs(future.diff(now, 'days').toObject().days);
